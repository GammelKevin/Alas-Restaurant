...
def statistics():
    ...
    # Berechnung des Datums vor 30 Tagen
    ...
    # Sammle alle Besuche für die letzten 30 Tage
    thirty_days_ago_date = today - timedelta(days=30)
    page_visits_raw = PageVisit.query.filter(PageVisit.timestamp >= thirty_days_ago_date).all()
    ...
    page_visits_query = db.session.query(
        ...
        PageVisit.timestamp >= thirty_days_ago
    ).group_by(
        PageVisit.page
    ).order_by(
    ...
    gallery_views_query = db.session.query(
        ...
        GalleryView.timestamp >= thirty_days_ago
    ).group_by(
